...
export const witnesses: Witnesses<PrivateState> = {
  authoritySecret: ({ privateState }) => {
    if (!privateState.authoritySecret) throw new Error('SR_UNAUTHORIZED');
    return [privateState, privateState.authoritySecret];
  },
  credentialOpening: ({ privateState }) => {
    if (!privateState.opening) throw new Error('SR_INVALID');
    return [privateState, privateState.opening];
  },
};
...
> {
  ...
  const unsupported = async (): Promise<never> => {
    throw new Error(
      'Private-state backup is unavailable; export the credential file locally.',
    );
  };
  return {
