...
export async function connectWallet(): Promise<ConnectedAPI> {
  ...
  const wallet: InitialAPI | undefined =
    wallets?.mnLace ??
    Object.values(wallets ?? {}).find((w) => w.apiVersion.startsWith('4.'));
  if (!wallet) throw new Error('WALLET_MISSING');
  const api = await wallet.connect(config.network);
  const status = mapWalletStatus(
    await api.getConnectionStatus(),
    config.network,
  );
  if (status !== 'connected')
    throw new Error(
      status === 'wrong-network' ? 'WRONG_NETWORK' : 'DISCONNECTED',
    );
  return api;
}
